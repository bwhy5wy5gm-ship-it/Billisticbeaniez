...
export default function AdminUpdatesPage() {
  ...
  async function handleSave() {
    ...
    const url = editing ? `/api/updates/${editing.id}` : "/api/updates";
    await fetch(url, {
      method,
              ...
              <div className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Update title" className="h-10" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Write your update..." rows={6} className="resize-none" />
                </div>
                <div>
                  <label className="text-sm font-medium mb-1.5 flex items-center gap-1.5">
          ...
          <Card className="border-2 border-dashed">
            <CardContent className="pt-5">
              <div className="text-center py-12">
                ...
                <p className="text-xs text-muted-foreground/60 mt-1">Create your first project update.</p>
              </div>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {update.photos && update.photos.length > 0 && (
                        <div className="flex gap-1.5 mt-2">
                          {update.photos.slice(0, 3).map((url, i) => (
                            <img key={i} src={url} alt="" className="h-10 w-10 rounded border object-cover" />
                          ))}
                          {update.photos.length > 3 && (
                            <span className="text-xs text-muted-foreground self-center">+{update.photos.length - 3}</span>
                          )}
                        </div>
                      )}
                    </div>
                    <div className="flex gap-1.5 shrink-0">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(update)}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20" onClick={() => handleDelete(update.id)}>
