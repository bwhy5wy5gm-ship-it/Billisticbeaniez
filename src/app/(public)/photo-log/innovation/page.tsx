...
export default function InnovationPhotosPage() {
  const [items, setItems] = useState<PhotoItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/photos?group=innovation")
      .then((r) => r.json())
      .then((data) => {
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold mb-3">Innovation Photos</h1>
          <p className="text-muted-foreground max-w-md mx-auto">
            Our innovative project solution and creative process.
          </p>
        </div>
        ...
        {loading ? (
            ...
            <CardContent className="pt-5">
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground font-medium mb-1">
                  No innovation photos yet
                </p>
                <p className="text-sm text-muted-foreground/60">
                  Project solution photos will appear here.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((item) => (
              <Link key={item.id} href={`/photo-log/innovation/${item.id}`}>
                <Card className="border-2 hover:border-emerald-200/80 dark:hover:border-emerald-800/50 transition-all hover:shadow-sm cursor-pointer group overflow-hidden h-full">
